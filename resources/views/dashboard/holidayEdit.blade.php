...
<style>
 ...
 #Textp{
    ...
    @endif
        <h2>Holiday</h2>
        <p>Please fill this form to edit holiday.</p>
        <form  method="post" action="{{ route('holidayUpdate', $holiday->id) }}">
              @csrf
            {{ method_field('GET') }}
            ...
            <div class="form-group" id ="Textp">
                <label>Date</label>
                <input type="date" name="date" min="1" class="form-control" value="{{ $holiday->date}}"  required>
            </div>  
